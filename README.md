...
事前に登録されたメールアドレスでログインするユーザーごとに、会話履歴・人物記憶を分離して保持します（アカウント作成機能はありません）。プライバシーに配慮し、会話全文はDBに保存せず、AIが生成した要約のみを保持します。また、登録なしで機能を一通り試せる「ゲストログイン」も用意しており、ゲストの会話・記憶・履歴はDBに一切保存されません（デモ・体験目的）。
...
* ゲストログイン（登録不要。会話・要約・人物記憶をDBに保存せず、その場限りで機能を試せる）
