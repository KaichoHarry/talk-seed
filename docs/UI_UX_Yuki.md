...
本システムのフロントエンドは、以下の6画面で構成する（実装では音声設定を独立画面としたため、当初案の5画面から1画面増えている）。

| No. | 画面名 | 役割 |
|---|---|---|
| 1 | トップ画面 | アプリの説明と開始 |
| 2 | シーン選択画面 | 場所・関係性・気分を選ぶ |
| 3 | 音声設定画面 | 音声タイプ・音量・話速を選ぶ（初回セットアップ時、以後は下部ナビ「設定」からも変更可） |
| 4 | 会話サポート画面 | 話題表示、音声読み上げ、マイクボタンでの発話、会話記録 |
| 5 | 会話まとめ画面 | AIによる要約、盛り上がった話題の表示 |
| 6 | 履歴画面 | 過去の会話ログを確認 |
...
- 話題カード
- 音声読み上げ状態
- マイクボタン（プッシュトゥトーク）と、認識した発話内容の表示
- もう一度読むボタン
- 深掘り質問ボタン
- 次の話題ボタン
- 会話を記録するボタン
...
音声設定は、会話サポート画面内の「音声設定」ボタンから開く。画面遷移は行わず、下から表示されるパネル、またはモーダルとして表示する。
...
| 項目 | 内容 |
|---|---|
| 音声タイプ変更 | 読み上げ音声の印象を変更する |
| 話速変更 | 読み上げの速さを変更する |
| 音量変更 | 読み上げ音声の音量を変更する |
| プレビュー再生 | 設定した音声を短い例文で確認する |
| 保存する | 設定内容を反映してパネルを閉じる |
| 閉じる | 変更せずにパネルを閉じる |

### 音声タイプの例

実装では以下の3種類（edge-ttsの音声＋ピッチ調整で実現）とした。

| 音声タイプ | 印象 | 実装 |
|---|---|---|
| 女性音声 | 一般的で聞き取りやすい | ja-JP-NanamiNeural |
| 男性音声 | 落ち着いた印象 | ja-JP-KeitaNeural |
| ロボット | 機械的な印象 | ja-JP-KeitaNeuralをベースにピッチ-45Hz・話速-15% |

### 話速設定の例

| 表示 | 値 | 説明 |
|---|---|---|
| ゆっくり | 0.75倍 | 落ち着いて聞きたい場合 |
| 標準 | 1.0倍 | 通常の読み上げ速度 |
| 少し速い | 1.25倍 | テンポよく使いたい場合 |
| 速い | 1.5倍 | 短時間で確認したい場合 |
...
- 音声設定は会話サポート画面内からすぐ開けるようにする
- 音声設定は画面遷移ではなく、パネル表示にして操作の流れを止めない
- 記録中でも話題カードは見えるようにする
...
| VoiceSettingsButton | 会話サポート画面 | 音声設定パネルを開く |
| VoiceSettingsPanel | 会話サポート画面 | 音声タイプ・話速・音量を変更する |
| VoiceTypeSelector | 音声設定パネル | 音声タイプを選ぶ |
| SpeedSlider | 音声設定パネル | 話速を調整する |
| VolumeSlider | 音声設定パネル | 音量を調整する |
| PreviewVoiceButton | 音声設定パネル | 設定した音声を試聴する |
...
会話サポート画面では、AIが話題を生成した直後に、話題を自動で読み上げる。ユーザーは音声設定から、読み上げ音声のタイプ、話速、音量を変更できる。
...
7. 必要に応じて「音声設定」から音声タイプ・話速・音量を調整する
```

### 音声設定の操作の流れ

```text
1. 会話サポート画面で「音声設定」を押す
2. 音声設定パネルが表示される
3. 音声タイプを選択する
4. 話速をスライダーで調整する
5. 音量をスライダーで調整する
6. 「プレビュー再生」で聞こえ方を確認する
7. 「保存する」を押して会話サポート画面に戻る
```
...
`sound-setting.html` のような専用画面は作成しない。音声設定は `talk.html` 内のパネルとして表示し、`talk.js` で状態を管理する。

---

## 14. JavaScriptの状態管理案

会話サポート画面では、記録状態と音声設定状態を変数で管理する。
...
| 状態 | 表示・動作 |
|---|---|
| voiceType = standard | 標準音声で読み上げる |
| voiceType = bright | 明るい音声で読み上げる |
| voiceType = calm | 落ち着いた音声で読み上げる |
| voiceType = clear | はっきりした音声で読み上げる |
| speechRate | 読み上げ速度に反映する |
| volume | 読み上げ音量に反映する |
...
5. 話題が自動で読み上げられる
6. 必要に応じて「音声設定」で音声タイプ・話速・音量を変更する
7. 必要に応じて「深掘り質問」や「次の話題」を使う
8. 会話を残したい場合は「会話を記録する」を押す
9. 同じ画面内でREC表示とタイマーが表示される
10. 「記録を終了する」を押す
11. 会話まとめ画面でAI要約を見る
12. 必要なら保存する
13. 履歴画面で過去の会話を確認する
...
フロントエンドは、トップ画面、シーン選択画面、会話サポート画面、会話まとめ画面、履歴画面の5画面で構成する。中心となる会話サポート画面では、AIの話題表示、音声読み上げ、音声設定、深掘り質問、次の話題、会話記録を1画面にまとめる。

音声設定機能では、音声タイプ変更、話速変更、音量変更を行えるようにする。これにより、飲食店やライブ会場など、利用場所に合わせて聞きやすい音声に調整できる。
